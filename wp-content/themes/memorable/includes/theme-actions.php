...
$default_google_fonts = array('Merriweather Sans');
	...
	function woo_custom_styling() {
		...
		if ( is_array($settings) ) {
			...
			if ( $settings['link_color'] != '' ) {
				$output .= 'a, #navigation #header-contact ul.nav li:before, #header .cart-contents:before { color: ' . $settings['link_color'] . ' !important; }' . "\n";
			}
			...
			if ( $settings['button_color'] != '' ) {
				$output .= 'a.button, #commentform #submit, .submit, input[type=submit], input.button, button.button, #wrapper .woo-sc-button, .added_to_cart { background: ' . $settings['button_color'] . ' !important; border-color: ' . $settings['button_color'] . ' !important; }' . "\n";
				$output .= 'a.button:hover, #commentform #submit:hover, .submit:hover, input[type=submit]:hover, input.button:hover, button.button:hover, #wrapper .woo-sc-button:hover, .added_to_cart:hover { background: ' . $settings['button_color'] . ' !important; opacity: 0.9; }' . "\n";
			}

			if ( $settings['button_text_color'] != '' ) {
				$output .= 'a.button, #commentform #submit, .submit, input[type=submit], input.button, button.button, #wrapper .woo-sc-button, .added_to_cart  { color: ' . $settings['button_text_color'] . ' !important; }' . "\n";
				$output .= 'a.button:hover, #commentform #submit:hover, .submit:hover, input[type=submit]:hover, input.button:hover, button.button:hover, #wrapper .woo-sc-button:hover, .added_to_cart:hover { color: ' . $settings['button_text_color'] . ' !important; }' . "\n";
			}

			if ( $settings['header_img'] != '' ) {
				$header_image = $settings['header_img'];
				if ( is_ssl() ) { $body_image = str_replace( 'http://', 'https://', $header_image ); }
				$output .= '#header { background-image: url( ' . esc_url( $header_image ) . ' ) !important; }' . "\n";
			}
		...
		if ( is_array($woo_options) ) {
			...
			if ( isset( $woo_options['woo_texttitle'] ) && $woo_options['woo_texttitle'] == 'true' ) {
				...
				if ( $woo_options['woo_font_site_title'] )
					...
					$output .= '#header .site-title a { color:' . $woo_options['woo_font_site_title']['color'] . '!important; }' . "\n";
...
if (!function_exists( 'appply_custom_fonts') ) {
	function appply_custom_fonts() {
		$font = "<link href='http://fonts.googleapis.com/css?family=Merriweather+Sans' rel='stylesheet' type='text/css'>
";
		if ( is_ssl() ) { $font = str_replace( 'http://', '//', $font ); }
		echo $font;
	}
}
...
function woo_load_prettyphoto () {
	...
	// Avoid loading prettyPhoto twice
	if ( is_woocommerce_activated() ) {
		$wc_lightbox = get_option( 'woocommerce_enable_lightbox' ) == 'yes' ? true : false;
		if ( is_woocommerce() && $wc_lightbox ) return;
	}
