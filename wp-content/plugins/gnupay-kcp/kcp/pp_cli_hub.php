...
$check_array = array('corp_type', 'good_name', 'buyr_name', 'buyr_mail', 'buyr_tel1', 'trad_time', 'amt_tot', 'amt_sup', 'amt_svc', 'amt_tax', 'tr_code', 'id_info', 'req_tx', 'pay_type', 'pay_trade_no');
...
if ($od_cash) {
    wp_die(__('이미 등록된 현금영수증 입니다.', GNUPAY_NAME));
}
...
<?php
    ...
    require_once(GNUPAY_KCP_PATH.'kcp/pp_cli_hub_lib.php');
    ...
    /* = -------------------------------------------------------------------------- = */
                ...
                update_post_meta($order_id, '_od_cash', 1);   // 메타데이터에 업데이트
                update_post_meta($order_id, '_od_cash_no', $cash_no);   // 메타데이터에 업데이트
                $result = update_post_meta($order_id, '_od_cash_info', $cash_info);   // 메타데이터에 업데이트
...
?>
    ...
    <form name="pay_info" method="post" action="<?php echo add_query_arg(array('wc-api'=>'gnupay_kcp_tax', 'tx'=>'pp_cli_result'), home_url( '/' )); ?>">
        <?php wp_nonce_field( 'kcp_cli_result', 'kcp_cli_nonce' ); ?>
        <input type="hidden" name="tx"  value='pp_cli_result'>
