...
if( ! defined( 'GNUPAY_NAME' ) ) exit; // 개별 페이지 접근 불가
	...
	$tran_cd        = isset($_POST["tran_cd"]) ? gp_check_array_sanitize($_POST[ "tran_cd"        ]) : ''; // 처리 종류
	/* = -------------------------------------------------------------------------- = */
    ...
    $tno            = isset($_POST["tno"]) ? gp_check_array_sanitize($_POST["tno"]) : ''; // KCP 거래 고유 번호
    /* = -------------------------------------------------------------------------- = */
    ...
    /* = -------------------------------------------------------------------------- = */
    $mod_type       = isset($_POST["mod_type"]) ? gp_check_array_sanitize($_POST["mod_type"]) : ''; // 변경TYPE VALUE 승인취소시 필요
    $mod_desc       = isset($_POST["mod_desc"]) ? gp_check_array_sanitize($_POST["mod_desc"]) : ''; // 변경사유
    /* = -------------------------------------------------------------------------- = */
	...
	$mobile_no      = "";                         // 휴대폰 번호
	/* = -------------------------------------------------------------------------- = */
	$tk_shop_id = $shop_user_id = isset($_POST["tk_shop_id"]) ? gp_check_array_sanitize($_POST["tk_shop_id"]) : ''; // 가맹점 고객 아이디
	$tk_van_code    = "";                         // 발급사 코드
	$tk_app_no      = "";                         // 상품권 승인 번호
	/* = -------------------------------------------------------------------------- = */
    $cash_yn        = isset($_POST["cash_yn"]) ? gp_check_array_sanitize($_POST["cash_yn"]) : ''; // 현금영수증 등록 여부
    $cash_authno    = "";                         // 현금 영수증 승인 번호
    $cash_tr_code   = isset($_POST["cash_tr_code"]) ? gp_check_array_sanitize($_POST["cash_tr_code"]) : ''; // 현금 영수증 발행 구분
    $cash_id_info   = isset($_POST["cash_id_info"]) ? gp_check_array_sanitize($_POST["cash_id_info"]) : ''; // 현금 영수증 등록 번호
    /* ============================================================================== */
    /* =   01-1. 에스크로 지불 요청 정보 설정                                       = */
    /* = -------------------------------------------------------------------------- = */
    $escw_used      = isset($_POST["escw_used"]) ? gp_check_array_sanitize($_POST["escw_used"]) : ''; // 에스크로 사용 여부
    $pay_mod        = isset($_POST[  "pay_mod"       ]) ? gp_check_array_sanitize($_POST["pay_mod"]) : ''; // 에스크로 결제처리 모드
    $deli_term      = isset($_POST[  "deli_term"     ]) ? gp_check_array_sanitize($_POST["deli_term"]) : ''; // 배송 소요일
    $bask_cntx      = isset($_POST[  "bask_cntx"     ]) ? gp_check_array_sanitize($_POST["bask_cntx"]) : ''; // 장바구니 상품 개수
    $good_info      = isset($_POST[  "good_info"     ]) ? gp_check_array_sanitize($_POST["good_info"]) : ''; // 장바구니 상품 상세 정보
...
if ( $req_tx == "pay" )
{
        ...
        $kcp_enc_data = apply_filters('gnupay_kcp_enc_data', $_POST[ "enc_data" ]);
        $kcp_enc_info = apply_filters('gnupay_kcp_enc_info', $_POST[ "enc_info" ]);

        $c_PayPlus->mf_set_encx_data( $kcp_enc_data, $kcp_enc_info );
}
